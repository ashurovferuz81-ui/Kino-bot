...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "subscribed":
        if str(user_id) in db["users"]:
            ...
            save_db(db)
            # Habarni yangilash va inline tugmani olib tashlash
            await query.edit_message_text("✅ Siz obuna bo‘ldingiz! Endi kino kodi yuboring.")
        return
    ...
    if query.data == "upload_movie":
        ...
        await update.message.reply_text("Iltimos kanal URL sini yuboring (https://t.me/…)")

# ---------------- ADMIN TEXT HANDLER ----------------
async def admin_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id

    # Agar foydalanuvchi bo‘lsa, kodni user_code_handler ga yuboradi
    if user_id != ADMIN_ID:
        await user_code_handler(update, context)
        return
    ...
    if state == "awaiting_code" and text:
        ...
        # video yoki document video qabul qilinadi
        video_file_id = None
...
async def user_code_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_subscribed(user_id):
        message_text = "🔒 Kanalga obuna bo‘lishingiz shart!"
        keyboard = [
            [InlineKeyboardButton("Obuna bo‘lish", url=channel_url)],
            [InlineKeyboardButton("Obuna buldim ✅", callback_data="subscribed")]
        ]
