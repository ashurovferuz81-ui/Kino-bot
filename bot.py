...
def is_admin_uploading(user_id):
    return db["admin_state"].get(str(user_id)) == "upload_movie"

def set_admin_upload_state(user_id, state):
    if state:
        db["admin_state"][str(user_id)] = "upload_movie"
    else:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "upload_movie":
        set_admin_upload_state(user_id, True)
        await query.message.reply_text("Kino kodi bilan video yuboring (caption: kodi)")
...
async def admin_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin_uploading(user_id):
        if update.message.video and update.message.caption:
            ...
            save_db(db)
            set_admin_upload_state(user_id, False)
            await update.message.reply_text(f"Kino saqlandi! Kod: {code}")
        else:
            await update.message.reply_text("Iltimos, video faylni caption bilan yuboring (kino kodi).")
...
def main():
    ...
    app.add_handler(MessageHandler(filters.VIDEO & filters.Caption, admin_text_handler))
